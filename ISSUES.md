...
- **Status:** PARTIALLY RESOLVED in PR-F (2026-04-06). Bumped the register helper timeout in `e2e/helpers/auth.ts` from 10s to 30s, which tolerates both CI (fast local Postgres) and local (slow remote Neon) latency profiles. Re-promoted the lefthook `e2e-fast` hook to blocking. The longer-term fix (point local at a real local Postgres test DB rather than relying on a longer timeout against the prod Neon) is still open and is the better answer for performance and isolation. Open as a P3 follow-up: build a `scripts/e2e-local-db.sh` that provisions a local Postgres test database, runs migrations, and is invoked by `e2e-precheck.sh`. Set `DATABASE_URL_E2E_LOCAL` and have playwright.config.ts prefer it over the prod connection string when both are present.
